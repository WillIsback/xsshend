...
impl FileSelectionScreen {
    pub fn render(f: &mut Frame, area: Rect, state: &AppState) {
        ...
        let current_path = Paragraph::new(format!("📂 {}", state.current_file_path))
            ...
            .block(Block::default().borders(Borders::ALL).title("Répertoire actuel"));
        f.render_widget(current_path, chunks[1]);

        // Liste des fichiers disponibles
        if let Ok(files) = state.get_current_directory_files() {
            let mut items = vec![
                ListItem::new("📁 ..").style(Style::default().fg(Color::Cyan))
            ];
            ...
            for file in &files {
                ...
                let icon = if state.selected_files.contains(file) { "✅" } else { "📄" };
                items.push(ListItem::new(format!("{} {}", icon, file_name)).style(style));
            }

            let files_list = List::new(items)
                .block(Block::default().borders(Borders::ALL).title("Fichiers (Espace pour sélectionner)"))
        ...
        let selected_text = if state.selected_files.is_empty() {
            Text::from("Aucun fichier sélectionné\n\nUtilisez les flèches ↑↓ pour naviguer\nAppuyez sur Espace pour sélectionner")
        } else {
            let mut lines = vec![Line::from(format!("📁 {} fichier(s) sélectionné(s):", state.selected_files.len()))];
            ...
            lines.push(Line::from(format!("📊 Taille totale: {}", 
    ...
    pub fn render(f: &mut Frame, area: Rect, state: &AppState) {
        ...
        if let Some(ref selector) = state.hierarchical_selector {
            ...
            let files_info = Paragraph::new(files_summary)
                .style(Style::default().fg(Color::Yellow))
                .block(Block::default().borders(Borders::ALL).title("Fichiers à téléverser"))
                .wrap(Wrap { trim: true });
            ...
            let error_msg = Paragraph::new("❌ Erreur: Sélecteur hiérarchique non initialisé\n\nAppuyez sur 'q' pour quitter")
...
impl DestinationInputScreen {
    pub fn render(f: &mut Frame, area: Rect, state: &AppState) {
        ...
        // Résumé des fichiers
        let total_size: u64 = state.selected_files.iter()
        ...
        let servers_info = Paragraph::new(servers_summary)
            .style(Style::default().fg(Color::Green))
            .block(Block::default().borders(Borders::ALL).title("Serveurs cibles"));
        ...
        let destination_input = Paragraph::new(destination_display)
            ...
            .block(Block::default().borders(Borders::ALL).title("Répertoire de destination"));
        ...
        let examples_text = "💡 Exemples de chemins :\n\
            ...
            • /etc/config/            (configuration)".to_string();
        
        let examples = Paragraph::new(examples_text)
            .style(Style::default().fg(Color::Gray))
            .block(Block::default().borders(Borders::ALL).title("Exemples de chemins de destination"))
            .wrap(Wrap { trim: true });
        f.render_widget(examples, chunks[4]);

        // Instructions de saisie
        let instructions_text = "📝 Saisie: Tapez votre chemin | Backspace: Effacer (ou retour si vide) | Esc: Vider\n🚀 Raccourcis: F1=/home | F2=/tmp | F3=/opt | Entrée/Tab: Continuer → | q: Quitter".to_string();
...
impl ProgressScreen {
    pub fn render(f: &mut Frame, area: Rect, state: &AppState) {
        ...
        let total_transferred: u64 = state.transfers.values()
            .map(|t| t.bytes_transferred)
            .sum();
        let total_size: u64 = state.transfers.values()
            .map(|t| t.total_bytes)
            .sum();
        ...
        let stats_text = format!(
            ...
            crate::ui::app_state::format_bytes(total_size),
            if total_size > 0 { (total_transferred as f64 / total_size as f64) * 100.0 } else { 0.0 },
            if total_speed > 0.0 { crate::ui::app_state::format_bytes(total_speed as u64) } else { "0 B".to_string() }
        );
        ...
        let header = Paragraph::new(stats_text)
            ...
            .block(Block::default().borders(Borders::ALL).title("État des transferts"));
        f.render_widget(header, chunks[0]);

        // Liste des transferts avec détails améliorés
        let mut transfer_items = Vec::new();
        for (host_name, progress) in &state.transfers {
            let progress_bar = Self::create_progress_bar(progress.bytes_transferred, progress.total_bytes);
            ...
            let speed_text = if progress.speed > 0.0 {
                format!(" - {}/s", crate::ui::app_state::format_bytes(progress.speed as u64))
            } else {
                String::new()
            };
            ...
            let status_icon = match progress.status {
                crate::ui::tui::TransferStatus::Pending => "⏳",
                crate::ui::tui::TransferStatus::Connecting => "🔄",
                crate::ui::tui::TransferStatus::Transferring => "📤",
                crate::ui::tui::TransferStatus::Completed => "✅",
                crate::ui::tui::TransferStatus::Failed => "❌",
                crate::ui::tui::TransferStatus::Paused => "⏸️",
            };
            ...
            if let Some(error) = &progress.error_message {
                ...
                transfer_items.push(ListItem::new(format!("   ↳ Erreur: {}", error))
                    .style(Style::default().fg(Color::Red)));
            } else {
                transfer_items.push(ListItem::new(line_text).style(item_style));
            }
        ...
        let transfers_list = List::new(transfer_items)
            .block(Block::default().borders(Borders::ALL).title("Détails des transferts"));
        ...
        let instructions_text = if state.are_all_transfers_complete() {
            "✅ Transferts terminés! | l: Basculer logs | q: Quitter | r: Relancer les échecs"
        } else {
            "⏸️ p: Pause/Reprendre | 📝 l: Basculer logs | ❌ q: Quitter"
        };
