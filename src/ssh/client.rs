...
impl SshClient {
    ...
    pub fn connect(&mut self) -> Result<()> {
        ...
        let (hostname, port) = if self.host.contains(':') {
            ...
            (parts[0], parts.get(1).unwrap_or(&"22").parse().unwrap_or(22))
        } else {
            (self.host.as_str(), 22)
        };
    ...
    pub fn upload_file(&mut self, local_path: &Path, remote_path: &str) -> Result<u64> {
        ...
        let mut local_file = std::fs::File::open(local_path)
            .with_context(|| format!("Impossible d'ouvrir le fichier local: {}", local_path.display()))?;
        ...
        F: FnMut(u64, u64) // (bytes_written, total_bytes)
    {
        ...
        let file_size = std::fs::metadata(local_path)
            .with_context(|| format!("Impossible de lire les métadonnées: {}", local_path.display()))?
            .len();
